...
import com.sdq.coupling.code.analysis.IPatternAnalysis;import com.sdq.coupling.code.analysis.PatternViolationType;
...
public class CognicryptPatternAnalysis implements IPatternAnalysis {
	...
	public List<AbstractPatternViolation> findViolations(String jarFilePath) {
		...
		try {
			HeadlessCryptoScanner hcs = HeadlessCryptoScanner.createFromOptions(new String[]{
					"--rulesDir", ".\\src\\main\\resources\\crypto-rules", 
					"--applicationCp", jarFilePath,
					"--reportDir", ".\\src\\main\\resources\\tmp", 
					"--sarifReport", ""});
			hcs.exec();
			
			//read txt file with result json 
			BufferedReader reader = new BufferedReader(new FileReader(".\\src\\main\\resources\\tmp\\CogniCrypt-SARIF-Report.txt"));
			String json = "";
			try {
			...
			for(int i = 0; i < resultsArray.length(); i++) {
				...
				CognicryptPatternViolation violation = 
						new CognicryptPatternViolation(errorLocation, violatedClassLocation, violationType, codeLines);
			...
			// TODO: clear tmp folder
				...
				| IllegalArgumentException | InvocationTargetException | ParseException | IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		/***
		 * hard coded violations for testing
		List<AbstractPatternViolation> violations = new LinkedList<AbstractPatternViolation>();
		//mocked violation for Cipher.getInstance("AES")
		Location errorLocation = new Location("com.sdq.example", "Client", "doSomething");
		Location violatedUsageLocation = new Location("com.sdq.example", "Cipher", "getInstance");
		List<Integer> codeLines = new LinkedList<Integer>();
		codeLines.add(17);
		violations.add(new CognicryptPatternViolation(errorLocation, violatedUsageLocation, PatternViolationType.ENCRYPTION, codeLines));
		*/
	...
	public class CogniCryptAdapter extends HeadlessCryptoScannerOptions {
	}
}
